...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget popularFoodItem(BuildContext context, ProductModel product,
    ...
    return GestureDetector(
      ...
      onTap: () async {
        //await FireStoreUtils.getAllProducts().then((value) => {print(value)});
        VendorModel? vendorModel =
  ...
  Widget buildCouponsForYouItem(BuildContext context1, OfferModel offerModel) {
    return
        // vendorModel == null
        //   ? Container()
        //   :
  ...
  Future<void> getData() async {
    offerList.clear();
    var listt = await FireStoreUtils().getAllCoupons();
    setState(() {
      for (var a in listt) {
        offerList.add(a);
      }
    ...
    fireStoreUtils.getRestaurantNearBy().whenComplete(() async {
      ...
      QuerySnapshot<Map<String, dynamic>> couponsQuery1 =
          await FirebaseFirestore.instance.collection('coupons').get();
      print(couponsQuery1);
      var lis;
      await FireStoreUtils.getAllProducts().then((value) => {lis = value});
      for (ProductModel p in lis) {
        //print(p.name);
      }
      setState(() {
        lstNearByFood = lis;
      ...
      lstAllRestaurant!.listen((event) {
        ...
        //   //   print(element1.discount);
        //   //   // event.forEach((element) {
        //   //   //   if (element1.expireOfferDate!.toDate().isAfter(DateTime.now())) {
        //   //   //     offersList.add(element1);
